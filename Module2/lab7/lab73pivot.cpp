...
//	int Index = rand() % ( Last - First + 1 ) + First;
	...
	static int Med[3];
	Med[0] = rand() % ( Last - First + 1 ) + First;
	Med[1] = rand() % ( Last - First + 1 ) + First;
	Med[2] = rand() % ( Last - First + 1 ) + First;
	static int Index;
	if ( arr[Med[0]] >= arr[Med[1]] && arr[Med[1]] >= arr[Med[2]])
	{
		if (arr[Med[1]] >= arr[Med[2]])
			Index = Med[1];
		else
			Index = Med[2];
	}
	...
	{
		if ( arr[Med[2]] >= arr[Med[1]] && arr[Med[2]] >= arr[Med[0]])
		{
			if (arr[Med[0]] >= arr[Med[1]])
				Index = Med[0];
			else
				Index = Med[1]; 
		}
